...
export interface AnalyticsBrowserSettings extends AnalyticsSettings {
  ...
  /**
   * If provided, will override the default Segment CDN (https://cdn.getastrolabe.com) for this application.
   */
  cdnURL?: string
}
...
async function registerPlugins(
  ...
  const shouldIgnoreSegmentio =
    (opts.integrations?.All === false && !opts.integrations['astrolabe.so']) ||
    (opts.integrations && opts.integrations['astrolabe.so'] === false)

  if (!shouldIgnoreSegmentio) {
    toRegister.push(
      segmentio(
        analytics,
        mergedSettings['astrolabe.so'] as SegmentioSettings,
