  ...
  describe('AJS', () => {
    test('fireEvent instantiates the right event types', async () => {
      ...
      expect(fetcher).toHaveBeenCalledWith(
        'https://webhook-dev.getastrolabe.com/v1/sdk/track',
        expect.anything()
      )

      await ajs.identify('identify')
      expect(fetcher).toHaveBeenCalledWith(
        'https://webhook-dev.getastrolabe.com/v1/sdk/identify',
        expect.anything()
      )

      await ajs.page('page')
      expect(fetcher).toHaveBeenCalledWith(
        'https://webhook-dev.getastrolabe.com/v1/sdk/page',
        expect.anything()
      )

      await ajs.group('group')
      expect(fetcher).toHaveBeenCalledWith(
        'https://webhook-dev.getastrolabe.com/v1/sdk/group',
        expect.anything()
      )

      await ajs.alias('alias')
      expect(fetcher).toHaveBeenCalledWith(
        'https://webhook-dev.getastrolabe.com/v1/sdk/alias',
        expect.anything()
      )

      await ajs.screen('screen')
      expect(fetcher).toHaveBeenCalledWith(
        'https://webhook-dev.getastrolabe.com/v1/sdk/screen',
        expect.anything()
      )
    })
  })
